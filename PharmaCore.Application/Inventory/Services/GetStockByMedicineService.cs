...
public class GetStockByMedicineService : IGetStockByMedicineService
{
    ...
    public async Task<ServiceResult<StockWithBatchesDto>> ExecuteAsync(int medicineId, CancellationToken cancellationToken = default)
    {
        try
        {
            var medicine = await _medicineRepository.GetByIdAsync(medicineId, cancellationToken);
            if (medicine is null)
        ...
        {
            _logger.LogError(e, "Error getting stock for medicine {MedicineId}", medicineId);
