...
ILogger<GetSalesStatementService> logger)
    : IGetSalesStatementService
{
    public async Task<ServiceResult<SalesStatementDto>> ExecuteAsync(int customerId, DateTime? from, DateTime? to, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                var salesBefore =
                    await saleRepository.GetByCustomerIdAsync(customerId, null, from.Value.AddTicks(-1),
                        cancellationToken);
                var saleIdsBefore = salesBefore.Select(s => s.SaleId).ToList();

                var paymentsBefore =
                    await paymentRepository.GetByReferencesAsync(PaymentReferenceType.SALE, saleIdsBefore,
                        cancellationToken);
                var returnsBefore = await salesReturnRepository.GetByCustomerIdAsync(customerId, null,
                    from.Value.AddTicks(-1), cancellationToken);
            ...
            var payments =
                await paymentRepository.GetByReferencesAsync(PaymentReferenceType.SALE, allSaleIds, cancellationToken);
            ...
            var returnsInRange =
                await salesReturnRepository.GetByCustomerIdAsync(customerId, from, to, cancellationToken);
        ...
        {
         string errMesage = $"Error getting customer statment: {e.Message}, {e.StackTrace}, {e.InnerException?.Message}";
         logger.LogError(e, errMesage);
         return ServiceResult<SalesStatementDto>.Fail(ServiceErrorType.ServerError, $"{errMesage}");
        }
    }
}
